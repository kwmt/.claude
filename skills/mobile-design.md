---
name: mobile-design
description: 高品質で独自性のあるモバイルインターフェースを作成する。Android（Jetpack Compose、XML）、iOS（SwiftUI、UIKit）、クロスプラットフォーム（Compose Multiplatform、Flutter）向けのモバイルコンポーネント、画面、アプリケーションの構築時に使用。テンプレート的な見た目を避けた、洗練されたコードとUIデザインを生成する。
license: Complete terms in LICENSE.txt
---

このスキルは、ありきたりな「AIっぽい」デザインを避けた、独自性のある本番品質のモバイルインターフェース作成をガイドする。美的な細部と創造的な選択に細心の注意を払い、実際に動作するコードを実装する。

ユーザーはモバイル要件を提供する：構築するコンポーネント、画面、アプリケーション、またはインターフェース。目的、ターゲットユーザー、プラットフォーム（Android/iOS/両方）、技術的制約についてのコンテキストを含む場合もある。
